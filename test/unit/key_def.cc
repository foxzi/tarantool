...
CFORMAT(printf, 1, 2) static struct tuple *
...
static void
test_key_compare_result(struct tuple *tuple_a, struct tuple *tuple_b,
			struct key_def *key_def, int expected,
			const char *funcname)
{
	const char *key_a = tuple_data(tuple_a);
	const char *key_b = tuple_data(tuple_b);
	mp_decode_array(&key_a);
	mp_decode_array(&key_b);
	size_t part_count = key_def->part_count;
	int rc = key_compare(key_a, part_count, HINT_NONE,
			     key_b, part_count, HINT_NONE,
			     key_def);
	ok(rc == expected, "%s(%s, %s) = %d, expected %d.", funcname,
	   tuple_str(tuple_a), tuple_str(tuple_b), rc, expected);
}
...
test_key_compare_singlepart(bool is_nullable)
{
	...
	for (size_t i = 0; i < tuples_eq.size(); i += 2) {
		...
		assert(tuple_field_count(a) == tuple_field_count(b));
		assert(tuple_field_count(a) == key_def->part_count);
...
test_key_compare(bool is_nullable)
{
	...
	if (is_nullable) {
		...
		tuples_eq.push_back(test_tuple_new("[NIL%u%u]", 0, 0));
		tuples_eq.push_back(test_tuple_new("[NIL%u%u]", 0, 0));

		tuples_eq.push_back(test_tuple_new("[%uNILNIL]", 0));
		tuples_eq.push_back(test_tuple_new("[%uNILNIL]", 0));

		tuples_eq.push_back(test_tuple_new("[NIL%uNIL]", 0));
		tuples_eq.push_back(test_tuple_new("[NIL%uNIL]", 0));

		tuples_eq.push_back(test_tuple_new("[NILNIL%u]", 0));
		tuples_eq.push_back(test_tuple_new("[NILNIL%u]", 0));

		tuples_eq.push_back(test_tuple_new("[NILNILNIL]"));
		tuples_eq.push_back(test_tuple_new("[NILNILNIL]"));
	}
	...
	if (is_nullable) {
		...
		tuples_gt.push_back(test_tuple_new("[%u%u%u]", 0, 0, 0));
		tuples_gt.push_back(test_tuple_new("[NILNILNIL]"));
		...
		tuples_gt.push_back(test_tuple_new("[%u%u%u]", 0, 0, 0));
		tuples_gt.push_back(test_tuple_new("[NILNIL%u]", 1));

		tuples_gt.push_back(test_tuple_new("[%u%u%u]", 0, 0, 0));
		tuples_gt.push_back(test_tuple_new("[NIL%u%u]", 1, 1));
...
test_tuple_compare_with_key_slowpath(bool is_nullable, bool has_optional_parts)
{
	...
	assert((has_optional_parts && is_nullable) || !has_optional_parts);
			    ...
			    bool is_unique)
{
	...
	if (is_unique) {
		/*
		 * It's assumed that PS and SK index different parts. So we
...
test_tuple_compare_sequential_nullable_with_optional_parts()
{
	...
	struct key_def *cmp_def = key_def_merge(key_def, pk_def);
	/*
	 * It's assumed that the PS covers one of SK's fields, so we can create
